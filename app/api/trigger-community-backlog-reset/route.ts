...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(`[trigger-community-backlog-reset] GitHub dispatch request failed: ${detail}`);
    return NextResponse.json({ error: 'GitHub dispatch request failed', detail }, { status: 504 });
  }

  if (!response.ok) {
    const body = await response.text().catch(() => '');
    console.error(`[trigger-community-backlog-reset] GitHub API error ${response.status}: ${body}`);
